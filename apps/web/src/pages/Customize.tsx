...
export default function Customize() {
  ...
  const handleAISubmit = async (data: { id: any; name: any;  Ingredients: any; Steps: any }) => {
    ...
    prompt += 'Choose the number of amount you want to add or reduce to adjust the recipe.'
    prompt += 'Return the recipe with the adapted ingredients and adapted steps. Use the JSON format.: "{ "Ingredients": {"Ingredients":[ ... ],"AmountOnePerson":[...]}, "Steps": [ ... ] }"\n'
    prompt += 'Only return the JSON file with no preleading or succesional text.'
  ...
  return (
    <>
      <div className="flex flex-col items-center my-4">
        <h1 className="text-2xl font-bold"> Customize -  RecipeID: {recipeID}</h1>
        <div className="dropdown">
          <label tabIndex={0} className="btn m-1">Spicy</label>
          <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52 flex items-center justify-center">
          <input type="range" min={-100} max="100" className="range" step="50" defaultValue="0" onChange={
            (e) => {
        ...
        <div className="dropdown mt-16">
          <label tabIndex={0} className="btn m-1">Cheese</label>
          <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52 flex items-center justify-center">
          <input type="range" min={-100} max="100" className="range" step="50" defaultValue="0" onChange={
            (e) => {
